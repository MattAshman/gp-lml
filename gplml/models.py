...
class LowerBoundLML(ExactMarginalLogLikelihood):
    def __init__(
        self,
        model: SGPR,
        logdet_estimator: Callable[[Tuple, CommonTerms], Tensor],
    ):
        if not isinstance(model, SGPR):
            raise ValueError(
                f"SGPR model expected in the constructor of the {self.__class__}"
            )
    ...
    def forward(self, data: Tuple[Tensor, Tensor], *params):
        ...
        return lower_bound, quad_term, logdet_term
    ...
    @abstractmethod
    def quad_estimator(self, data, terms) -> Tensor:
        pass


class LowerBoundCG(LowerBoundLML):
    def __init__(
        self,
        model: CGLB,
        logdet_estimator: Callable[[Tuple, CommonTerms], Tensor],
        ...
        if not isinstance(model, CGLB):
            raise ValueError(
                f"CGLB model expected in the constructor of the {self.__class__}"
            )

        super().__init__(model, logdet_estimator)

        self.cg_opt = ConjugateGradient() if cg_opt is None else cg_opt
        self._cached_v_vec = cached_v_vec_initial

        # TODO: do not use cached vactor.
...
class PredictCG(LowerBoundCG):
    def __init__(
        ...
        super().__init__(model, logdet_estimator, cg_opt)
...
class PredictSGPR(LowerBoundSGPR):
    def forward(
        self, xnew: Tensor, full_cov: bool = False, full_output_cov: bool = False
    ) -> Tuple[Tensor, Tensor]:
        ...
        trisolve = torch.triangular_solve
        c = trisolve(a_err, terms.LB, upper=False).solution / sigma
        tmp1 = trisolve(delazify(kus), terms.L, upper=False).solution
        tmp2 = trisolve(tmp1, terms.LB, upper=False).solution

        f_mean = tmp2.transpose(-1, -2) @ c + self.mean(xnew).reshape(-1, 1)
...
class PredictLogdensityLML(LowerBoundLML):
    def forward(
        self,
...
def logdet_estimator_thang(
    ...
    """
    Bound from Jensen's Inequality:
    log |K + σ²I| <= log |Q + σ²I| +  Σ_n log(1 + (K - Q)_{nn}/σ²)

    :param terms: Common terms. LB, and A is used.
    :return: log_det, upper bound on .5 * log|K + σ²I|
    """
    ...
    # AtA = Q / σ2.
    AtA = terms.A.transpose(-1, -2) @ terms.A
    AtA_diag = AtA.diagonal()

    # d / σ2.
    t = kdiag / sigma_sq - AtA_diag
    ...
    # Correction term from Jensen's inequality
    logdet -= 0.5 * output_dim * torch.log(1.0 + t).sum()
    return logdet
...
def logdet_estimator_sgpr(
    ...
    """
    Bound from Jensen's Inequality:
    log |K + σ²I| <= log |Q + σ²I| + tr(K - Q) / (nσ2)

    :param terms: Common terms. AAt is used.
    :return: log_det, upper bound on .5 * log|K + σ²I|
    """
...
def logdet_estimator_exact(
    ...
    :param terms: Common terms. None are used.
    :return log_det .5 * log|K + σ²I|
    """
